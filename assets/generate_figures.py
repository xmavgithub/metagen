...
def generate_pareto_front():
    ...
    ax.plot(params[pareto_idx], score[pareto_idx], "r--", alpha=0.5, linewidth=2)
...
def generate_convergence():
    ...
    # Evolutionary
    ax.fill_between(generations, evo_mean - evo_std, evo_mean + evo_std, alpha=0.2, color="crimson")
    ax.plot(
        generations,
        evo_mean,
        "s-",
...
def generate_benchmark_radar():
    ...
    # Draw the chart
    ax.plot(angles, metagen_scores, "o-", linewidth=2, label="MetaGen", color="crimson")
    ax.fill(angles, metagen_scores, alpha=0.25, color="crimson")

    ax.plot(angles, baseline_scores, "s-", linewidth=2, label="Baseline", color="steelblue")
...
def generate_modality_overview():
    ...
    bars1 = ax.bar(
        x - width / 2, architectures, width, label="Architecture Families", color="steelblue"
    )
    bars2 = ax.bar(x + width / 2, example_specs, width, label="Example Specs", color="forestgreen")
