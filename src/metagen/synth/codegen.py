...
def generate_code(spec: ModelSpec, out_dir: Path, blueprint: BlueprintState, seed: int) -> None:
    ...
    model_code = env.get_template("model.py.j2").render(
        spec=spec, blueprint=blueprint, dropout=dropout
    )
    train_code = env.get_template("train.py.j2").render(spec=spec, blueprint=blueprint)
    data_code = env.get_template("data.py.j2").render(spec=spec, blueprint=blueprint)
    eval_code = env.get_template("eval.py.j2").render(spec=spec, blueprint=blueprint)
    infer_code = env.get_template("infer.py.j2").render(spec=spec, blueprint=blueprint)
